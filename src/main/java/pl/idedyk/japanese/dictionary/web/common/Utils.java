...
public class Utils {
	...
	public static ThemeType getTheme(HttpServletRequest httpServletRequest) {
		
		// pobieramy motyw z ciasteczka		
		...
		for (Cookie cookie : cookies) {
			if (cookie.getName().equals("theme") == true) { // mamy ciastko
				...
				// zamieniamy na enum-a
				return Arrays.asList(ThemeType.values()).stream().filter(f -> f.themeValue.equals(themeValue)).findFirst().orElse(ThemeType.LIGHT);				
			}
		}
	...
	public static void setTheme(HttpServletResponse httpServletResponse, HttpSession httpSession, ThemeType themeType) {
		Cookie cookie = new Cookie("theme", themeType.themeValue);
		...
		httpSession.setAttribute("theme", themeType.themeValue);
	}
	
	private static String getAutocompleteHistoryKey(String autocompleteName) {
